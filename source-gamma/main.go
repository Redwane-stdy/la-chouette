// source-gamma — Producteur Kafka pour les équations assignées à la source "gamma".
//
// Identique à source-alpha dans sa logique, mais :
//   - assigned_source = "gamma" dans PostgreSQL
//   - Intervalle contrôlé par SOURCE_GAMMA_INTERVAL_MS (défaut: 5000ms)
//   - Logs en ORANGE avec préfixe [GAMMA]
//
// Comportement :
...
//  4. Quand toutes les équations sont publiées, log "Source GAMMA terminée" et exit proprement.
//
// Variables d'environnement :
//   - KAFKA_BROKERS            : adresse(s) du broker Kafka (défaut: kafka:9092)
//   - POSTGRES_DSN             : DSN PostgreSQL complet
//   - SOURCE_GAMMA_INTERVAL_MS : intervalle entre publications en ms (défaut: 5000)
...
type Config struct {
	...
	SourceName   string        // "gamma" — nom de la source dans les messages Kafka
	KafkaTopic   string        // topic cible, "equations"
}

// loadConfig lit les variables d'environnement et retourne la configuration.
func loadConfig() Config {
	intervalMs := int64(5000) // défaut 5 secondes pour gamma
	if v := os.Getenv("SOURCE_GAMMA_INTERVAL_MS"); v != "" {
...
// ─────────────────────────────────────────────────────────────────────────────
// Logging — couleur ORANGE pour [GAMMA]
// ─────────────────────────────────────────────────────────────────────────────

// Codes ANSI pour la couleur orange (jaune doré en ANSI standard) et le reset.
const (
	colorOrange = "\033[33m" // ANSI yellow est rendu orange dans la plupart des terminaux
	colorReset  = "\033[0m"
)

// setupLogger configure zerolog avec le niveau demandé et une sortie console
// colorisée. Le préfixe [GAMMA] est injecté en orange dans chaque message.
func setupLogger(level string) {
	// Sortie console avec couleurs et timestamp lisible
	output := zerolog.ConsoleWriter{
		Out:        os.Stdout,
		TimeFormat: "2006-01-02T15:04:05.000Z",
		// Ajout du préfixe coloré [GAMMA] au formatage du message
		FormatMessage: func(i interface{}) string {
			return fmt.Sprintf("%s[GAMMA]%s %s", colorOrange, colorReset, i)
		},
	}
...
func connectKafka(ctx context.Context, brokers string, topic string) (*kafka.Writer, error) {
	...
	// Vérification que le broker est joignable via une connexion directe
	maxKafkaAttempts := 20
	var lastErr error
...
// pour la source "gamma" dans PostgreSQL. Retente toutes les 3 secondes.
...
func main() {
	...
	log.Info().Msgf("%s[GAMMA]%s En attente du démarrage — ouvrez http://localhost:3000 et cliquez START", colorOrange, colorReset)
	for {
		select {
		case <-ctx.Done():
			return
		...
		if err != nil {
			...
			log.Info().Msgf("%s[GAMMA]%s Expérience démarrée ! Publication des équations...", colorOrange, colorReset)
			break
		} else {
			log.Info().Msgf("%s[GAMMA]%s En attente du signal START sur http://localhost:3000 ...", colorOrange, colorReset)
		}
	...
	log.Info().Msgf("✓ Toutes les équations publiées. Source GAMMA terminée.")
}
